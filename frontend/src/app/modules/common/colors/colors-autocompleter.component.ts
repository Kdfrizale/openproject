...
export class ColorsAutocompleter implements OnInit {
  ...
  ngOnInit() {
    ...
    this.coloredObject = this.elementRef.nativeElement.dataset.coloredobject;
  ...
  private highlightColor(item:any) {
    ...
    if(this.coloredObject === 'type') {
      highlightingClass = '__hl_inline_type_ ';
      highlightingType = 'text';
    } else {
      highlightingClass = '__hl_inline_ ';
      highlightingType = 'dot';
    }
    return highlightingClass + Highlighting.colorClass(highlightingType, item.value);
  }
